...
NB = 'assets/notebooks/bidirectional/02_databricks_metric_view.ipynb'
nb = json.load(open(NB))
src = next(''.join(c['source']) for c in nb['cells']
           if c['cell_type'] == 'code' and 'def shim_snowflake_to_converter' in ''.join(c['source']))
ast.parse(src)
print("notebook 02 shim cell parses")
for token in ("def pop_synonyms", "def keep_synonyms_as_extension",
              "keep_synonyms_as_extension(metric)", '"ai_context"] = {"synonyms"'):
    print(f"  present: {token!r} -> {token in src}")

shim_only = src.split('# ------------------------------------------------------------ the two directions')[0]
ns = {}
exec(shim_only, ns)
s2c, c2s = ns['shim_snowflake_to_converter'], ns['shim_converter_to_snowflake']
...
print()
print("NOTEBOOK 02 SHIM OK" if good else "PROBLEM")
